...
function JailLogin()
	...
	ESX.ShowNotification("В последний раз, когда вы заснули, вы были заключены в тюрьму, из-за этого вы сейчас снова в тюрьме!")
...
function UnJail()
	...
	ESX.ShowNotification("Вы освобождены, оставайтесь спокойны снаружи! удачи!")
end
...
function InJail()
	...
	Citizen.CreateThread(function()
		...
		while jailTime > 0 do
			...
			ESX.ShowNotification("Вас посодили на " .. jailTime .. " минут в тюрьму!")
					...
					if DistanceCheck <= 1.5 then
						...
						if IsControlJustPressed(0, 38) then
							...
							if v["state"] then
								...
								ESX.ShowNotification("Вы уже забрали эту посылку!")
							end
		...
		while true do
			...
			for p, v in pairs(Config.Teleports) do
				...
				if DistanceCheck <= 7.5 then
					...
					ESX.Game.Utils.DrawText3D(v, "[E] Открыть дверь", 0.4)
...
function PackPackage(packageId)
	...
	while Packaging do
		...
		if PackPercent >= 100 then
			...
			ESX.Game.Utils.DrawText3D(Package, "Упаковка... " .. math.ceil(tonumber(PackPercent)) .. "%", 0.4)
		end
...
function DeliverPackage(packageId)
	...
	while Packaging do
		...
		if not IsEntityAttachedToEntity(packageId, PlayerPedId()) then
			...
			ESX.Game.Utils.DrawText3D(DeliverPosition, "[E] Доставил посылку", 0.4)
...
function OpenJailMenu()
	ESX.UI.Menu.Open(
		'default', GetCurrentResourceName(), 'jail_prison_menu',
		{
			...
			align    = 'center',
			elements = {
				{ label = "Посадить", value = "jail_closest_player" },
				{ label = "Выпустить", value = "unjail_player" }
			}
		}, 
	function(data, menu)
          	...
          	function(data2, menu2)
            	...
            	if jailTime == nil then
              		...
              		if closestPlayer == -1 or closestDistance > 3.0 then
                		ESX.ShowNotification("Поблизости нет игрока!")
					else
						ESX.UI.Menu.Open(
							'dialog', GetCurrentResourceName(), 'jail_choose_reason_menu',
							{
							  title = "Причина задержания"
							},
						function(data3, menu3)
						  	...
						  	if reason == nil then
								ESX.ShowNotification("Ты должен положить сюда что-нибудь!")
						  	else
								menu3.close()
		...
		elseif action == "unjail_player" then
			...
			ESX.TriggerServerCallback("esx-qalle-jail:retrieveJailedPlayers", function(playerArray)
				...
				if #playerArray == 0 then
					ESX.ShowNotification("Твоя тюрьма пуста!")
					return
				end

				for i = 1, #playerArray, 1 do
					table.insert(elements, {label = "Освободить: " .. playerArray[i].name .. " | Срок: " .. playerArray[i].jailTime .. " Минут", value = playerArray[i].identifier })
				end
