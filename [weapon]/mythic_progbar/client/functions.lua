...
function Process(action, start, tick, finish)
    ...
    if not IsEntityDead(PlayerPedId()) or mythic_action.useWhileDead then
        if not isDoingAction then
            ...
            TriggerEvent('mythic_notify:client:SendAlert:custom', PlayerPedId(), { type = 'error', text = tostring(mythic_action.duration), length = 1000})
            ...
            TriggerEvent('mythic_notify:client:SendAlert:custom', PlayerPedId(), { type = 'error', text = 'Already Doing An Action', length = 1000})
            -- exports['mythic_notify']:SendAlert('error', 'Already Doing An Action', 1000)
        end
    else
        TriggerEvent('mythic_notify:client:SendAlert:custom', PlayerPedId(), { type = 'error', text = 'Cannot Perform An Action While Dead', length = 1000})
