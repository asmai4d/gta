  ...
  function Start(ped)
	  ...
	  while checking do
		  ...
		  if distance < 2.0 then
			  DrawText3D(x,y,z, 'Нажмите [~g~E~s~] чтобы проверить причины смерти', 0.4)
  ...
  function Notification(x,y,z)
	  ...
	  while (timestamp + 4500) > GetGameTimer() do
		  Citizen.Wait(0)
		  DrawText3D(x, y, z, 'Похоже, здесь происходят повреждения.', 0.4)
		  checking = false
	  end
  end
  ...
  function OpenDeathMenu(player)
	  ...
	  table.insert(elements, {label = 'Выявление причины смерти', value = 'deathcause'})
	  table.insert(elements, {label = 'Определить место, где произошел ущерб', value = 'damage'})
	  ...
	  function(data, menu)
		  ...
		  if ac == 'damage' then
			  ...
			  if success then
				  ...
				  Notify('Причиненный ущерб определить не удалось')
			  end
		  end
