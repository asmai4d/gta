...
function fishshop()
	...
	local elements = {}
	table.insert(elements, {label = 'Приобрести удочку - <span style="color:green;">200$</span>', value = 'buyrod'})
	if Config.licenses then
		if not ownedLicenses['fishing'] then
			table.insert(elements, {label = 'Приобрести разрешение на ловлю рыбы - <span style="color:green;">250$</span>', value = 'buylice'})
		else
			table.insert(elements, {label = 'Вы уже владеете <span style="color:lightblue;">Разрешения на рыбалку</span>', value = 'exit'})
		end
	end
	table.insert(elements, {label = 'Exit', value = 'exit'})
    ESX.UI.Menu.Open(
        'default', GetCurrentResourceName(), 'рыболовный магазин',
    {
...
Citizen.CreateThread(function()
	...
	local w = 1000
        if dist < 5 then
            Draw3DText2(x, y, z + 0.5, "[~r~E~w~] Открытый магазин | [~g~G~w~] Продавайте своих рыбок")
            if IsControlJustReleased(0, 38) then 
                if ClockTime >= 6 -1 and ClockTime < 22 then
                    fishshop()
                else
                    TriggerEvent('mythic_notify:client:slerba', { type = 'error', length = 7000, text = 'Магазин в настоящее время недоступен' })
                end
...
Citizen.CreateThread(function()
	...
	AddTextComponentString('Рыбный рынок') -- set blip's "name"
