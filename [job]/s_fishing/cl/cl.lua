    ...
    if IsEntityInWater(PlayerPedId()) or IsEntityInWater(boat) then
        if not IsPedInAnyVehicle(PlayerPedId(), false) then
            if not IsPedSwimming(PlayerPedId()) then
                ...
                draw = true
                rng22 = true
                ESX.ShowNotification('Рыбалка...')
                TriggerEvent('s_fishing:rng')
                fishing()
            else
                ESX.ShowNotification('Вы должны решить, хотите ли вы плавать или ловить рыбу.')
            end
        end
    else
        ESX.ShowNotification('Я не могу ловить здесь рыбу...')
    end
end)
...
function drawtext()
    ...
    while draw do
        ...
        Draw3DText(pedcoords.x - 1, pedcoords.y, pedcoords.z + 1,'Нажмите [~r~X~s~] прекратить рыбалку')
        if IsControlJustPressed(0, 73) then
            ESX.ShowNotification('Прекратил рыбалку')
            DeleteEntity(rod)
            DeleteObject(rod)
            rng22 = false
            TriggerEvent('s_fishing:rng')
    ...
    if rng22 then
        TriggerEvent('s_fishing:minigame', function(success)
            if success then
                ...
                ClearPedTasks(PlayerPedId())
                TriggerEvent('s_fishing:rng')
                ESX.ShowNotification("Вы ничего не поймали, попробуйте еще раз")
            end
        end)
    end
end
