...
local blips = {
	...
	{title="GS Customs", colour=-1, id=446, x =-210.68, y = -1323.58, z = 26.85,}, -- Location to obtain supplies needed to make cocaine at Drug Processing Location
  }
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'groove_actions', {
		...
		if data.current.value == 'vehicle_list' then
			...
			if Config.EnableSocietyOwnedVehicles then
				...
				local elements = {						
					{label = '"Эвокуатор"', value = 'flatbed3'},					
				}
...
function OpenMobilegrooveActionsMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'mobile_groove_actions', {
		...
		align    = 'top-left',
		elements = {
			{label = '📝 Выстовить счет',       value = 'billing'},
			{label = '👨‍🏭 Вызовите механика',       value = 'meca1'},					
			{label = '🔨 Вскрытие',        value = 'hijack_vehicle'},
			{label = '🛠️ Ремонт',        value = 'fix_vehicle'},
			{label = '🧽 Мойка',         value = 'clean_vehicle'},
			{label = '🔐 Конфисковать',       value = 'del_vehicle'},
			{label = '⚙️ Погрузка/Разгрузка',      value = 'dep_vehicle'},
			{label = '📦 Объекты', value = 'object_spawner'}			
		}
	}, function(data, menu)
	...
	elseif data.current.value == 'object_spawner' then
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'mobile_groove_actions_spawn', {
			...
			align    = 'top-left',
			elements = {			
				{label = 'Машинная вода', value = 'prop_watercooler_dark'},				
				{label = 'Забор', value = 'prop_barrier_work06a'},				
				{label = 'Ремонт', value = 'imp_prop_impexp_postlift'},	
				{label = 'Тележка', value = 'prop_cs_trolley_01'},					
				{label = 'Настенное колесо', value = 'imp_prop_impexp_hub_rack_01a'},				
				{label = 'Председатель', value = 'v_ret_gc_chair02'},			
				{label = _U('дорожный камень'), value = 'prop_roadcone02a'},
				{label = _U('ящик для инструментов'),  value = 'prop_toolchest_01'}
			}
