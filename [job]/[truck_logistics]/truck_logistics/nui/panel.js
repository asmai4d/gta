	...
	if (item.showmenu){
		...
		list_item = `
			<div class="col-12 mt-3 mb-1">
				<h4 class="text-uppercase">ДИЛЕРСКИЙ ЦЕНТР</h4>
				<p>Покупайте больше грузовиков для вас и ваших водителей</p>
			</div>`;

		for (const key in config.concessionaria) {
			if (config.concessionaria[key].rank <= rank) {
					...
					<div class="card"> <img src="` + truck.img + `" class="card-img-top" width="100%">
						<div class="card-body pt-0 px-0">
							...
							<div class="d-flex flex-row justify-content-between p-3 mid">
								<div class="d-flex flex-column"><small class="text-muted mb-1">ДВИГАТЕЛЬ</small>
								...
								<div class="d-flex flex-column"><small class="text-muted mb-2">МОЩНОСТЬ</small>
									<div class="d-flex flex-row"><img src="img/power_icon.jpg">
										<h6 class="ml-1">` + truck.hp + ` hp</h6>
									</div>
								</div>
							</div>
							<div class="mx-3 mt-3 mb-2"><button onclick="buyTruck('` + key + `','` + truck.price + `')" type="button" class="btn btn-blue btn-darken-2 white btn-block"><small>ВЗЯТЬ</small></button></div> <small class="d-flex justify-content-center text-muted">*Legal Disclaimer</small>
						</div>
					</div>
					`;
			}
