...
Citizen.CreateThread(function()
    ...
    while ESX.GetPlayerData().job3 == nil do Wait(0) end
    PlayerData = ESX.GetPlayerData()
    if Config.Command then
        RegisterCommand(Config.Command, function()
            TriggerEvent('esx_vagosdog:openMenu')
        end)
    end
    while true do
        local sleep = 250
...
RegisterNetEvent('esx_vagosdog:openMenu')
AddEventHandler('esx_vagosdog:openMenu', function()
    mainMenu()
end)

RegisterNetEvent('esx_vagosdog:hasDrugs')
AddEventHandler('esx_vagosdog:hasDrugs', function(hadIt)
    if hadIt then
...
mainMenu = function()
    if PlayerData.job3.name == Config.Job3[1] or PlayerData.job3.name == Config.Job3[2] then
        ...
        ESX.UI.Menu.Open(
                'default', GetCurrentResourceName(), 'buy_storage',
            {
                ...
                align = 'top-left',
                elements = {{label = Strings['take_out_remove'], value = 'take_out_remove'}, {label = Strings['sit_stand'], value = 'sit_stand'}, {label = Strings['search_drugs'], value = 'search_drugs'}, {label = Strings['attack_closest'], value = 'attack_closest'}, {label = Strings['small'], value = 'small'}}
            },
            function(data, menu)

      if data.current.value == 'put_in_vehicle' then
              TriggerEvent('esx_ambulancejob:putInVehicle')  
              menu.close()
      end
            ...
            ESX.TriggerServerCallback('esx_ambulancejob:getItemAmount', function(quantity)
              if quantity > 0 then
                ...
                if health > 0 then
                  ...
                  IsBusy = true
                  ESX.ShowNotification('You put a bandage')
                  ...
                  ESX.ShowNotification('You have finished', GetPlayerName(closestPlayer))
                  IsBusy = false
                else
                  ESX.ShowNotification('player_not_conscious')
                end
              else
                ESX.ShowNotification('you have no bandage')
              end
            end, 'bandage')
                            ...
                            if GetDistanceBetweenCoords(GetEntityCoords(policeDog), GetEntityCoords(PlayerPedId()), true) <= 3.0 then
                                ...
                                if distance ~= -1 then
                                    if distance <= 3.0 then
                                        local playerPed = GetPlayerPed(player)
                                        if not IsPedInAnyVehicle(playerPed, true) then
                                            TriggerServerEvent('esx_vagosdog:hasClosestDrugs', GetPlayerServerId(player))
                                        end
                                    end
                                end
                            end
