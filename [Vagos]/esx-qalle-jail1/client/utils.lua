RegisterCommand("jailmenu", function(source, args)
	...
	if PlayerData.job3.name == "vagos" then
		OpenJailMenu()
	else
		ESX.ShowNotification("You are not an vagos!")
	end
end)
...
function Cutscene()
	...
	LoadModel(-1773333796)

	local VagosPosition = Config.Cutscene["VagosPosition"]
	local Vagos = CreatePed(5, -1773333796, VagosPosition["x"], VagosPosition["y"], VagosPosition["z"], VagosPosition["h"], false)
	TaskStartScenarioInPlace(Vagos, "WORLD_HUMAN_PAPARAZZI", 0, false)
	...
	DeleteEntity(Vagos)
	SetModelAsNoLongerNeeded(-1773333796)
