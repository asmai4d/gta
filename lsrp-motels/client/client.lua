				...
				if motel and room then
                    if motel ~= nil and room ~= nil then
                        print('loading saved room 2')
                            ...
                            ESX.ShowNotification("ROOM RECEIVED FROM DB: " .. roomID)
                            print("ROOM RECEIVED FROM DB: " .. roomID)
                            ...
                            if roomID ~= nil then
                                ...
                                print("Creating room")
                                local instanceid = 'motel'..tostring(roomID)..''..tostring(roomIdent)
...
--         }
        ...
        ESX.ShowNotification("Cancel rent")
        if not status then

            roomOwner = nil
            
            ESX.ShowNotification("Cancel rent (not status)")
...
AddEventHandler('instance:onEnter', function(instance)
    ...
    if instance.type == 'motelroom' then
        ...
        ESX.ShowNotification("instance:onEnter")
        ...
        ESX.ShowNotification("instance.type ~= motelroom")
...
function EnterProperty(name, owner, motel, room)
    ...
    -- ESX.ShowNotification("Saving room: " .. curRoom)
    ...
    ESX.ShowNotification("EnteredProperty: " .. playerInstance.data.room)
...
AddEventHandler('lsrp-motels:enterRoom', function(room, motel)
    ...
    roomID = roomno
            ...
            ESX.ShowNotification("lsrp-motels:enterRoom ROOM: " .. roomID)
            ...
            if playerInstance ~= nil then
                ...
                TriggerEvent('instance:enter', playerInstance)     
            else
                print("Instance not ready. Creating.")
                if roomID ~= nil then
                    local instanceid = 'motel'..roomID..''..roomIdent
                        TriggerEvent('instance:create', 'motelroom', {property = instanceid, owner = ESX.GetPlayerData().identifier, motel = reqmotel, room = roomIdent, vid = roomID})
...
AddEventHandler('lsrp-motels:exitRoom', function(motel, room)
    ...
    TriggerEvent('instance:leave', playerInstance)
end)
    ...
    for k,v in pairs(Config.Zones) do
        for kk,vm in pairs(v.Rooms) do       
            if room == vm.instancename then
                motelName = v.Name
                motelRoom = vm.number
                motelShortName = v.ShortName
            end
        end
    end

    print("Renting room: " .. room)

    TriggerServerEvent('lsrp-motels:SaveMotel', motelShortName, room)
...
--         }
    ...
    ESX.ShowNotification("Rent")

        if not status then

            ESX.ShowNotification("Rent (not status)")
...
function ExitProperty(name, motel, room)
    ...
    ESX.ShowNotification("1Exiting room: " .. room)
        ...
        for k,v in pairs(Config.Zones) do
            for km,vm in pairs(v.Rooms) do
                if room == vm.instancename then
                    ...
                    ESX.ShowNotification("2Exiting room: " .. vm.instancename)
    ...
    ESX.ShowNotification("ExitProperty")
