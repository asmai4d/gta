...
AddEventHandler('lsrp-motels:rentRoom', function(room)
	...
	local id = MySQL.Sync.fetchAll("SELECT id FROM lsrp_motels WHERE ident=@ident", {['@ident'] = playerIdent})

	TriggerClientEvent('esx:showNotification', src, "!Room!")

    if id and id[1] then
		TriggerClientEvent('esx:showNotification', src, "Room found")
	else
		TriggerClientEvent('esx:showNotification', src, "Room NOT found")
