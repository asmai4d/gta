RegisterCommand("jailmenu", function(source, args)
	...
	if PlayerData.job3.name == "famillies" then
		OpenJailMenu()
	else
		ESX.ShowNotification("You are not an famillies!")
	end
end)
...
function Cutscene()
	...
	LoadModel(1309468115)

	local FamilliesPosition = Config.Cutscene["FamilliesPosition"]
	local Famillies = CreatePed(5, 1309468115, FamilliesPosition["x"], FamilliesPosition["y"], FamilliesPosition["z"], FamilliesPosition["h"], false)
	TaskStartScenarioInPlace(Famillies, "WORLD_HUMAN_PAPARAZZI", 0, false)
	...
	DeleteEntity(Famillies)
	SetModelAsNoLongerNeeded(1309468115)
