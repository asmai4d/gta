...
AddEventHandler("esx:playerLoaded", function(newData)
	...
	ESX.TriggerServerCallback("esx-qalle-jail2:retrieveJailTime", function(inJail, newJailTime)
		if inJail then
...
RegisterNetEvent("esx-qalle-jail2:openJailMenu")
AddEventHandler("esx-qalle-jail2:openJailMenu", function()
	OpenJailMenu()
end)

RegisterNetEvent("esx-qalle-jail2:jailPlayer")
AddEventHandler("esx-qalle-jail2:jailPlayer", function(newJailTime)
	jailTime = newJailTime

	Cutscene()
end)

RegisterNetEvent("esx-qalle-jail2:unJailPlayer")
AddEventHandler("esx-qalle-jail2:unJailPlayer", function()
	jailTime = 0
...
function InJail()
	...
	Citizen.CreateThread(function()
		...
		while jailTime > 0 do
			...
			TriggerServerEvent("esx-qalle-jail2:updateJailTime", jailTime)

			if jailTime == 0 then
				UnJail()

				TriggerServerEvent("esx-qalle-jail2:updateJailTime", 0)
			end
		...
		if not IsEntityAttachedToEntity(packageId, PlayerPedId()) then
			...
			if DistanceCheck <= 2.0 then
				if IsControlJustPressed(0, 38) then
					...
					TriggerServerEvent("esx-qalle-jail2:prisonWorkReward")
				end
			end
		end
...
function OpenJailMenu()
	...
	function(data, menu)
						...
						function(data3, menu3)
						  	...
						  	if reason == nil then
								...
								if closestPlayer == -1 or closestDistance > 3.0 then
								  	...
								  	TriggerServerEvent("esx-qalle-jail2:jailPlayer", GetPlayerServerId(closestPlayer), jailTime, reason)
								end
		...
		elseif action == "unjail_player" then
			...
			ESX.TriggerServerCallback("esx-qalle-jail2:retrieveJailedPlayers", function(playerArray)
