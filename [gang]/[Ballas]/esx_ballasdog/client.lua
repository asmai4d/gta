...
RegisterNetEvent('esx_ballasdog:openMenu')
AddEventHandler('esx_ballasdog:openMenu', function()
    mainMenu()
end)

RegisterNetEvent('esx_ballasdog:hasDrugs')
AddEventHandler('esx_ballasdog:hasDrugs', function(hadIt)
    if hadIt then
                            ...
                            if GetDistanceBetweenCoords(GetEntityCoords(policeDog), GetEntityCoords(PlayerPedId()), true) <= 3.0 then
                                ...
                                if distance ~= -1 then
                                    if distance <= 3.0 then
                                        local playerPed = GetPlayerPed(player)
                                        if not IsPedInAnyVehicle(playerPed, true) then
                                            TriggerServerEvent('esx_ballasdog:hasClosestDrugs', GetPlayerServerId(player))
                                        end
                                    end
                                end
                            end
