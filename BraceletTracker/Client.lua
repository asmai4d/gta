...
AddEventHandler('Cyber:AddBraceletTrackerForPlayer',
    ...
    if isjobshare then
        ESX.ShowNotification(
            '~g~One of Your Colleges Added A Tracker For You. Check Your Map, Label : ~y~' ..
                labell)
    else
        ESX.ShowNotification(
            'Tracker Activated Succesfuly Check Your Map, Label :  ~y~' .. labell)
    end
end)
            ...
            if not status then
                ...
                if label and label ~= '' then
                    local boundary = GetUserInput(
                                         'Enter boundary 0-' ..
                                             tostring(Config.Maximumboundary),
                                         15)
                    ...
                    if tonumber(boundary) >= Config.Maximumboundary then
                        return ESX.ShowNotification(
                                   'Please Enter a Lower Number as boundary. Maximum : ' ..
                                       tostring(Config.Maximumboundary))
                    end
                    ...
                    TriggerEvent("mythic_progbar:client:progress", {
                        ...
                        TriggerServerEvent('Cyber:TrackedUsedOnPlayer',
                                           GetPlayerServerId(closestPlayer),
                                           boundary, label, jobshare)
                ...
                return ESX.ShowNotification(
                           'Target Has a Tracking Bracelet Already.')
            end
                ...
                TriggerEvent("mythic_progbar:client:progress", {
                    ...
                    if not status then
                        ...
                        TriggerServerEvent('Cyber:BoltCutterUsedOnPlayer',
                                           GetPlayerServerId(closestPlayer))
                        ESX.ShowNotification(
                            '~g~ Succesfully Removed The Tracker.')
                    end
                end)

            else
                return ESX.ShowNotification(
                           'Target Does Not Have Tracking Bracelet.')
            end
        end, GetPlayerServerId(closestPlayer))
    end
end)
...
AddEventHandler('Cyber:BoltCutterUsedForPlayer', function(trgt)
   local target = trgt
    if Bracelets[target] then
        ESX.ShowNotification(
            'Someone has Cutt a Ankle Tracking Bracelet. Target Label : ' ..
...
AddEventHandler('Cyber:PlayerLeftTheServer', function(target)
    if Bracelets[target] then
        if Config.InformIfTargetLeavedServer then
            ESX.ShowNotification(
                'Player with Ankle Tracking Bracelet Left The Server. Target Label : ' ..
                    Bracelets[target].label)
        end
        RemoveBlip(Bracelets[target].blip)
        Bracelets[target] = nil
            ...
            for i, v in pairs(Bracelets) do
                if not v.breakboundary then
                    ...
                    if distance > tonumber(v.boundary) then
                        ESX.ShowNotification(
                            '~y~' .. v.label ..
                                '~r~ leaved The Specified boundary With Active Ankle Tracking Bracelet')
                        PlaySoundFrontend(-1, "DELETE",
                                          "HUD_DEATHMATCH_SOUNDSET", 1)
...
AddEventHandler('Cyber:LabelIsAlreadyBeingUsed', function(isjobshare, name)
    ...
    if isjobshare then
        msg = '~y~' .. name ..
                  '~r~ With Same Job With You Tried to add Tracking Ankle Bracelet for you but you Already had same label'
    else
        msg =
            '~r~You have same label for another person. Please use another label'
    end
