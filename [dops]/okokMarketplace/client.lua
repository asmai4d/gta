		...
		while true do
				...
				if distance < v.radius + 2 then
					...
					if distance < v.radius and height < 3 and height > -3 then
						...
						if not Config.UseOkokTextUI and not isMarketOpen then
							ESX.ShowHelpNotification('~INPUT_CONTEXT~ Чтобы открыть торговую площадку')
						end
						...
						if IsControlJustReleased(0, Config.OpenMarketKey) then
							if GetVehiclePedIsUsing(PlayerPedId()) == 0 then
								...
								exports['okokNotify']:Alert("CRAFTING", "Вы не можете получить доступ к рынку внутри транспортного средства", 5000, 'error')
							end
						end
					end
			...
			if inZone and not hasEntered then
				if Config.UseOkokTextUI then
					exports['okokTextUI']:Open('[E] Чтобы открыть торговую площадку', 'darkblue', 'left') 
				end
				hasEntered = true
...
RegisterNUICallback("action", function(data, cb)
	...
	elseif data.action == "missing" then
		exports['okokNotify']:Alert("MARKETPLACE", "Пожалуйста, заполните все необходимые поля", 5000, 'error')
	elseif data.action == "missingSearch" then
		exports['okokNotify']:Alert("MARKETPLACE", "Поле поиска пустое", 5000, 'error')
	elseif data.action == "high" then
		exports['okokNotify']:Alert("MARKETPLACE", "У вас недостаточно товаров для продажи", 5000, 'error')
