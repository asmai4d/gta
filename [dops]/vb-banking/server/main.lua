	...
	if zPlayer ~= nil then
		...
		if tonumber(_source) == tonumber(to) then
			...
			if balance <= 0 or balance < tonumber(amountt) or tonumber(amountt) <= 0 then
				TriggerClientEvent('chatMessage', _source, "У вас недостаточно денег на вашем банковском счете.")
			else
		...
		TriggerClientEvent('chatMessage', _source, "Этот идентификатор кошелька недействителен или не существует")
	end
end)