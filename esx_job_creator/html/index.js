...
function deleteRank(jobName, rankId) {
	deleteDialog("Delete rank", `Вы уверены, что хотите удалить это звание?`, function () {
...
function fillJobRanks(jobName, ranks) {
	...
	var mainRowContent = $(`
		<th scope="col">Имя рангаth>
		<th scope="col">Номер ранга</th>
		<th scope="col">Метка ранга</th>
		<th scope="col">Зарплата</th>
	`)

	if (!hasAnyRank) {
		mainRowContent = $(`<th scope="col">Званий для этой работы пока нет</th>`)
	}
	...
	ranks.forEach(rank => {
		...
		jobRank.click(function () {
			inputDialog("Edit job rank", [
				{ id: "rankName", label: "Имя ранга:", default: rank.name, type: "text", isId: true },
				{ id: "rankGrade", label: "Класс:", default: rank.grade, type: "number" },
				{ id: "rankLabel", label: "Метка ранга:", default: rank.label, type: "text" },
				{ id: "rankSalary", label: "Должностной оклад:", default: rank.salary, type: "number" },
				{
					id: "deleteRank", label: "Удалить ранг", type: "button", buttonType: "btn-danger",
...
function createRank(jobName) {
	inputDialog("Create new rank", [
		{ id: "rankName", label: "Имя ранга (id):", type: "text", isId: true },
		{ id: "rankGrade", label: "Класс:", type: "number" },
		{ id: "rankLabel", label: "Метка ранга:", type: "text" },
		{ id: "rankSalary", label: "Должностной оклад:", type: "number" },
	], function (data) {
...
$("#create-job-btn").click(function () {
	inputDialog("Create new job", [
		{ id: "jobLabel", label: "Новая метка работы:" },
		{ id: "jobName", label: "Новае названия работы(id):", isId: true },
	], function (data) {
	...
	for (const [_, job] of Object.entries(jobs)) {
		...
		dropdownEditJob.click(function () {
			...
			inputDialog("Edit job", [
				{ id: "jobName", label: "Новая работа (id):", default: jobName, type: "text", isId: true },
				{ id: "jobLabel", label: "Новая метка:", default: jobLabel, type: "text" },
				{ id: "enableBilling", label: "Включить выставление счетов", default: enableBilling, type: "switch" },
				{ id: "canRob", label: "Можно ли грабить/обыскивать игроков?", default: canRob, type: "switch" },
				{ id: "canHandcuff", label: "Можно ли надевать наручники на игроков?", default: canHandcuff, type: "switch" },
				{ id: "whitelisted", label: "Занесен в белый список", default: whitelisted, type: "switch" },
				{ id: "canLockpickCars", label: "Умеете взламывать замки машин?", default: canLockpickCars, type: "switch" },
				{ id: "canWashVehicles", label: "Можно ли мыть транспортные средства?", default: canWashVehicles, type: "switch" },
				{ id: "canRepairVehicles", label: "Умеете ремонтировать транспортные средства?", default: canRepairVehicles, type: "switch" },
				{ id: "canImpoundVehicles", label: "Можно ли конфисковывать транспортные средства?", default: canImpoundVehicles, type: "switch" },
				{ id: "canCheckIdentity", label: "Можете проверить личность?", default: canCheckIdentity, type: "switch" },
				{ id: "canCheckVehicleOwner", label: "Можете проверить владельца транспортного средства?", default: canCheckVehicleOwner, type: "switch" },
				{ id: "canCheckDrivingLicense", label: "Можете проверить водительские права?", default: canCheckDrivingLicense, type: "switch" },
				{ id: "canCheckWeaponLicense", label: "Можете проверить лицензию на оружие?", default: canCheckWeaponLicense, type: "switch" },
				// Delete job button
				{
					id: "deleteJob", label: "Удалить работу", type: "button", buttonType: "btn-danger",
					buttonCallback: function (modal) {
						modal.modal("hide");
						deleteJob(jobName);
					}
				...
				$.post(`https://${resName}/update-job`, JSON.stringify(data), function (resultData) {
					...
					if (resultData.isSuccessful) {
						...
						mainJobDiv.data("cancheckdrivinglicense", data.canCheckDrivingLicense);
						mainJobDiv.data("cancheckweaponlicense", data.canCheckWeaponLicense);
...
function getMarkerTypeOptionsLabel(markerType) {
	...
	let labels = {
		['crafting_table']: "Рецепты крафта",
		['shop']: "Настройка товаров магазина",
		['garage']: "Установка временных транспортных средств",
		['garage_buyable']: "Установка гаражных транспортных средств",
		['garage_owned']: "Установка гаража",
		['boss']: "Меню боса",
		['job_outfit']: "Гардероб",
		['teleport']: "Настройка телепортов",
		['market']: "Настройка продаваемых товаров",
		['harvest']: "Редактировать собираемый элемент",
		['process']: "Редактировать обрабатываемый элемент",
		['weapon_upgrader']: "Редактировать цены на компоненты",
		['job_shop']: "Редактировать параметры магазина",
	}
...
function createJobMarker(jobName) {
	inputDialog("Create marker", [
		{
			id: "markerType", label: "Тип маркера:", type: "select", options: [
				{ value: "stash", label: "Припрятать" },
				{ value: "armory", label: "Оружейка" },
				{ value: "safe", label: "Сейф" },
				{ value: "garage", label: "Гараж - Временные транспортные средства" },
				{ value: "garage_buyable", label: "Транспортные средства, которые можно купить в гараже" },
				{ value: "garage_owned", label: "Транспортные средства, находящиеся в гараже" },
				{ value: "boss", label: "Босс" },
				{ value: "wardrobe", label: "Гардероб - сохраненные игроком наряды" },
				{ value: "job_outfit", label: "Рабочая экипировка" },
				{ value: "shop", label: "Магазин (покупка товаров)" },
				{ value: "market", label: "Рынок (продажа товаров)" },
				{ value: "harvest", label: "Точка сбора урожая" },
				{ value: "process", label: "Точка процесса" },
				{ value: "crafting_table", label: "Стол для крафта" },
				{ value: "teleport", label: "Точка телепортации" },
				{ value: "weapon_upgrader", label: "Модернизация оружия" },
				{ value: "duty", label: "На дежурстве/Вне дежурства" },
			]
		},
		...
		{
			id: "currentCoordsBtn", label: "Текущие координаты", type: "button", buttonType: "btn-success",
			buttonCallback: function (modal) {
				$.post(`https://${resName}/get-current-coords`, {}, function (data) {
					var coords = data.coords;
		...
		{ id: "markerMinGrade", label: "Минимальный ранг:", type: "number", default: 0 },
	], function (data) {
		data.jobName = jobName
	...
	inputDialog("Create public marker", [
		{
			id: "markerType", label: "Тип маркера:", type: "select", options: [
				...
				{ value: "garage_owned", label: "Транспортные средства, находящиеся в гараже" },
				{ value: "wardrobe", label: "Гардероб - сохраненные игроком наряды" },
				{ value: "job_outfit", label: "Общественный Наряд" },
				{ value: "shop", label: "Магазин (покупка товаров)" },
				{ value: "market", label: "Рынок (продажа товаров)" },
				{ value: "harvest", label: "Точка сбора урожая" },
				{ value: "process", label: "Точка Процесса" },
				...
				{ value: "teleport", label: "Точка телепортации" },
				{ value: "weapon_upgrader", label: "Модернизация оружия" },
				{ value: "job_shop", label: "Магазин вакансий" },
			]
		},
		{ id: "label", label: "Label:", type: "text", default: "Default" },
		{ id: "markerCoordsX", label: "Coord X:", type: "number" },
