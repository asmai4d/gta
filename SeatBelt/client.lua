...
-- Import from Accident plugin
      vehicle = GetVehiclePedIsIn(PlayerPedId(-1), false)
      if not SeatbeltON and DoesEntityExist(vehicle) and (InVehicle or IsCar(vehicle)) then
        InVehicle = true
          ...
          if currentDamage ~= oldBodyDamage then
            if (oldBodyDamage - currentDamage) >= 25 or (oldSpeed - currentSpeed)  >= 35 then
              oldBodyDamage = currentDamage
      ...
      if not SeatbeltON and speedBuffer[2] ~= nil and GetEntitySpeedVector(car, true).y > 1.0 and speedBuffer[1] > (Config.Speed / 3.6) and (speedBuffer[2] - speedBuffer[1]) > (speedBuffer[1] * 0.255) then
        ...
        if not CanPedRagdoll(ped) then
          if Config.Notification then
            Notify(Config.TestStrings.can_not_fly)
          end
        end

        if CanPedRagdoll(ped) then
          if Config.Notification then
            Notify(Config.TestStrings.time_to_fly)
          end
          SetPedToRagdoll(ped, 1000, 1000, 0, 0, 0, 0)
        end
      end
