...
RegisterNetEvent('esx_familliesdog:openMenu')
AddEventHandler('esx_familliesdog:openMenu', function()
    mainMenu()
end)

RegisterNetEvent('esx_familliesdog:hasDrugs')
AddEventHandler('esx_familliesdog:hasDrugs', function(hadIt)
    if hadIt then
                            ...
                            if GetDistanceBetweenCoords(GetEntityCoords(policeDog), GetEntityCoords(PlayerPedId()), true) <= 3.0 then
                                ...
                                if distance ~= -1 then
                                    if distance <= 3.0 then
                                        local playerPed = GetPlayerPed(player)
                                        if not IsPedInAnyVehicle(playerPed, true) then
                                            TriggerServerEvent('esx_familliesdog:hasClosestDrugs', GetPlayerServerId(player))
                                        end
                                    end
                                end
                            end
