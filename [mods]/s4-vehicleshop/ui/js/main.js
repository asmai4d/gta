...
$(window).ready(function() {
    window.addEventListener('message', (event) => {
        ...
        if (data.action == "load"){
            ...
            intv = setInterval(() => {
                curTest = curTest - 1;
                $("#delivery-time").html("Test Surusu<br/>Bitmesine "+curTest+" saniye kaldi");
            }, 1000);
...
function loadCat(x) {
   ...
   $.each(garage.Vehicles[carCategories[x]], function (key, val) {
	   ...
	   if(key == 0) {
		  active = `active`;
		  $.post("https://s4-vehicleshop/showCar", JSON.stringify({ name: val.name }), function(x){});
          setPrice(val.price);
          $("#vehicle-name").html(val.name);
    

          $("#tank-lt").html(val.fuel + " LITRE" );
...
$(document).on('click', '.vehicle-list-name', function(e){
    ...
    $.post("https://s4-vehicleshop/showCar", JSON.stringify({ name: $(this).data("name") }), function(x){});
...
function testdrive(){
            $("#wrap").hide();
            $("body").hide();
			$.post("https://s4-vehicleshop/testdrive", JSON.stringify({}), function(x){});
}
...
$(document).on('click', '.choose-color-circle', function(e){
    ...
    $.post("https://s4-vehicleshop/setcolour", JSON.stringify({rgb: rgb}), function(x){});
});
...
function plakaonay() { 
    ...
    $.post("https://s4-vehicleshop/checkPlatePrice", JSON.stringify({plate: plate}), function(x){
...
function buy() {
   ...
   $.post("https://s4-vehicleshop/buy", JSON.stringify({blackMoney: blackMoney}), function(x){ 
        if(x == true) {  
            $("#wrap").hide();
            $("body").hide();
			//$.post("https://s4-vehicleshop/close", JSON.stringify({}), function(x){});
...
$(document).ready(function() {
    $('.colorpicker').minimalColorpicker({
        ...
        onUpdateColor: function(e, color) {
            $.post("https://s4-vehicleshop/setcolour", JSON.stringify({rgb: color.rgb}), function(x){});
        }
    });
});
